...
for dataset in "${dataset_list[@]}"; do
    for model in "${model_list[@]}"; do
            ...
            sbatch --job-name=emo-rec --mail-user=andreas.peintner@uibk.ac.at --time=300:00:00 --mem=64G --gres=gpu:1 ~/jobs/single-node-gpu.job "conda run -n elliot python run_sep.py --data $dataset --model $model"
            sleep 1
    done
done
