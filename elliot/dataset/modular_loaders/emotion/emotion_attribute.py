...
class EmotionAttribute(AbstractLoader):
    ...
    def create_namespace(self) -> SimpleNamespace:
        ns = SimpleNamespace()
        ns.__name__ = "EmotionAttribute"
        ns.object = self
        ns.emotion_feature_folder_path = self.emotion_feature_folder_path

        ns.item_mapping = self.item_mapping

        ns.emotion_features_shape = self.emotion_features_shape
    ...
    def get_all_features(self):
        return self.get_all_emotion_features()

    def get_all_emotion_features(self):
        all_features = np.empty((len(self.items), self.emotion_features_shape))
        if self.emotion_feature_folder_path:
            for key, value in self.item_mapping.items():
                all_features[value] = np.load(self.emotion_feature_folder_path + '/' + str(key) + '.npy')
        return all_features
