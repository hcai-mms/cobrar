...
class Evaluator(object):
    def __init__(self, data: ds.DataSet, params: SimpleNamespace):
        ...
        self._nonaveraged_results = True
    ...
    def eval(self, recommendations):
        ...
        result_dict = {}
        for k in self._k:
            val_results, val_statistical_results, val_nonaveraged, test_results, test_statistical_results, test_nonaveraged = self.eval_at_k(recommendations, k)
            local_result_dict ={"val_results": val_results,
                                "val_statistical_results": val_statistical_results,
                                "val_nonaveraged": val_nonaveraged,
                                "test_results": test_results,
                                "test_statistical_results": test_statistical_results,
                                "test_nonaveraged": test_nonaveraged,}
    ...
    def eval_at_k(self, recommendations, k):
        ...
        for p, (test_data, eval_objs) in enumerate(self._get_test_data()):
            if eval_objs is not None:
                eval_objs.cutoff = k
            results, statistical_results, nonaveraged_results = self._process_test_data(recommendations[p], test_data, eval_objs, val_test[p])
            result_list.append((results, statistical_results, nonaveraged_results))

        # print(list(result_list[0][2]['nDCG'].values()), type(result_list[1][2]))
        if (not result_list[0][0]):
            # If there are no validation data, return twice the test data..?
            return result_list[1][0], result_list[1][1], result_list[1][2], result_list[1][0], result_list[1][1], result_list[1][2]
        elif (not result_list[1][0]):
            # If there are no test data, return twice the validation data..?
            return result_list[0][0], result_list[0][1], result_list[0][2], result_list[0][0], result_list[0][1], result_list[0][2]
        else:
            return result_list[0][0], result_list[0][1], result_list[0][2], result_list[1][0], result_list[1][1], result_list[1][2]
    ...
    def _process_test_data(self, recommendations, test_data, eval_objs, val_test):
        if (not test_data) or (not eval_objs):
            ...
            nonaveraged_results = {m.name(): m.eval_user_metric() for m in metric_objects} if self._nonaveraged_results else {}
            ...
            return results, statistical_results, nonaveraged_results
